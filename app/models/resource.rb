class Resource < ActiveRecord::Base
  ...
  def self.project_detail_name(date,project)
    self.where("project = ? AND date >= ? AND date < ?" ,project, date, date +15.months).order("date").group("name","function").order("department","name","function").select("department, name, function, project")
  end
